...
layui.use('table', function(){
   ...
   table.render({
        ...
        ,cols: [[
            ...
            ,{field:'name', title:'公司名称', width:"25%"}
            ,{field:'address', title:'公司地址', width:"25%"}
            ,{field:'mark', title:'公司备注', width:"25%"}
            ,{fixed: 'right', title:'操作', toolbar: '#barDemo', width:"25%"}
        ]]
        ,page:true
        //异步请求响应
        ,url: '/organization/company/companyListPage'
        ,request: {
...
function member_del(id){
    let url = "/organization/company/deleteACompany?companyId="+id;
...
layui.use('form', function(){
    ...
    form.on('submit(sreach)', function(data){
        let [someCompanyName,someCompanyAddress] = [$("#someCompanyName").val(),$("#someCompanyAddress").val()]
        //都为空 则是普通执行
...
function initTable(someCompanyName,someCompanyAddress) {
    ...
    layui.use('table', function(){
        ...
        table.render({
            ...
            ,page:true
            //异步请求响应
            ,url: '/organization/company/likeSearch?someCompanyName='+someCompanyName+"&someCompanyAddress="+someCompanyAddress
            ,request: {
                pageName: 'pageNum' //页码的参数名称，默认：page
                ,limitName: 'pageSize' //每页数据量的参数名，默认：limit
            }
